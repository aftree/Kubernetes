...
is safer and more flexible than putting it verbatim in a
 term_id="pod" >}} definition or in a  text="container image" term_id="image" >}}. See [Secrets design document](https://git.k8s.io/community/contributors/design-proposals/auth/secrets.md) for more information.
-->
...
A secret can be used with a pod in two ways: as files in a
 text="volume" term_id="volume" >}} mounted on one or more of
...
[KubeletConfiguration struct](https://github.com/kubernetes/kubernetes/blob/docsbranch/staging/src/k8s.io/kubelet/config/v1beta1/types.go)).
...
for secret data, so that the secrets are not stored in the clear into etcd.
-->
