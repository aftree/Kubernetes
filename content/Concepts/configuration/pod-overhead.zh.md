...
<!-- 
Once a Pod is scheduled to a node, the kubelet on that node creates a new  text="cgroup" term_id="cgroup" >}}
for the Pod. It is within this pod that the underlying container runtime will create containers. -->
一旦 Pod 调度到了某个节点， 该节点上的 kubelet 将为该 Pod 新建一个  text="cgroup" term_id="cgroup" >}}. 底层容器运行时将在这个 pod 中创建容器。
