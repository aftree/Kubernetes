...
如果您使用 lossary_tooltip term_id ="deployment">}} 来运行您的应用程序，则它可以动态创建和销毁 Pod。
...
by a  text="selector" term_id="selector" >}}
...
这一组 `Pod` 能够被 `Service` 访问到，通常是通过 _tooltip text="selector" term_id="selector" >}}
...
you can query the sary_tooltip text="API server" term_id="kube-apiserver" >}}
...
如果您想要在应用程序中使用 Kubernetes 接口进行服务发现，则可以查询 ary_tooltip text="API server" term_id="kube-apiserver" >}} 的 endpoint 资源，只要服务中的Pod集合发生更改，端点就会更新。
...
实例:
  ...
  * 希望服务指向另一个 lossary_tooltip term_id="namespace" >}} 中或其它集群中的服务。
...
because lossary_tooltip term_id="kube-proxy" >}} doesn't support virtual IPs
as a destination.
-->
...
updates a global allocation map in ltip term_id="etcd" >}}
...
<!--
You can also use oltip term_id="ingress" >}} in place of Service
to expose HTTP / HTTPS Services.
-->
您还可以使用oltip term_id="ingress" >}} 代替 Service 来公开HTTP / HTTPS服务。
