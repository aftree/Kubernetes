...
<!--
A network policy is a specification of how groups of _tooltip text="pods" term_id="pod">}} are allowed to communicate with each other and other network endpoints.

NetworkPolicy resources use  text="labels" term_id="label">}} to select pods and define rules which specify what traffic is allowed to the selected pods.
-->

网络策略（NetworkPolicy）是一种关于  text="Pod" term_id="pod">}} 间及与其他网络端点间所允许的通信规则的规范。

NetworkPolicy 资源使用  text="标签" term_id="label">}} 选择 Pod，并定义选定 Pod 所允许的通信规则。
