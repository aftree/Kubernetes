...
, or perhaps several Pods, to carry out
...
### 通过 API 服务器来控制 {#control-via-API-server}
 ...
 控制器是一个 Kubernetes 内置控制器的例子。内置控制器通过和集群 API 服务器交互来管理状态。
...
control-plane中的其它组件根据新的消息而反应（调度新的 Pod 并且运行它）并且最终完成工作。
...
Kubernetes 自带有一组内置的控制器，运行在 kube-controller-manager} 内。这些内置的控制器提供了重要的核心功能。
